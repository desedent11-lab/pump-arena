...
def _fake_enemy() -> dict[str, Any]:
    fake_names = ["WhaleHunter", "Liquidator", "ShadowMaker", "CryptoWolf", "PumpKing", "MarketShark"]
    return {
        "telegram_id": "bot_" + str(random.randint(1000, 9999)),
        "username": random.choice(fake_names),
        "rating": random.randint(100, 1200),
        "is_bot": True,
    }


def _find_player_match(telegram_id: str):
...
def _create_match(player_a: dict[str, Any], player_b: dict[str, Any]) -> dict[str, Any]:
    ...
    audit("match_created", player_a.get("telegram_id", ""), {"match_id": match_id, "players": [player_a, player_b]})
    return match


@app.post("/api/matchmaking/join")
def matchmaking_join(req: MatchmakingJoinRequest) -> dict[str, Any]:
    player_data = {
        "telegram_id": req.telegram_id,
    ...
    existing_match_id, existing_match = _find_player_match(req.telegram_id)
    if existing_match:
        enemy = [p for p in existing_match["players"] if p.get("telegram_id") != req.telegram_id][0]
        return {"ok": True, "status": "matched", "match_id": existing_match_id, "enemy": enemy, "match": existing_match}

    # Remove duplicate queue entries for the same player.
    MATCHMAKING_QUEUE[:] = [p for p in MATCHMAKING_QUEUE if p.get("telegram_id") != req.telegram_id]

    if MATCHMAKING_QUEUE:
        enemy = MATCHMAKING_QUEUE.pop(0)
        match = _create_match(player_data, enemy)
        return {"ok": True, "status": "matched", "match_id": match["match_id"], "enemy": enemy, "match": match}

    MATCHMAKING_QUEUE.append(player_data)
    audit("matchmaking_join", req.telegram_id, {"username": req.username, "rating": req.rating})
    return {"ok": True, "status": "searching", "queue_size": len(MATCHMAKING_QUEUE)}


@app.get("/api/matchmaking/status/{telegram_id}")
def matchmaking_status(telegram_id: str) -> dict[str, Any]:
    match_id, match = _find_player_match(telegram_id)
    if match:
        enemy = [p for p in match["players"] if p.get("telegram_id") != telegram_id][0]
        return {"ok": True, "status": "matched", "match_id": match_id, "enemy": enemy, "match": match}

    in_queue = any(p.get("telegram_id") == telegram_id for p in MATCHMAKING_QUEUE)
    return {"ok": True, "status": "searching" if in_queue else "idle", "queue_size": len(MATCHMAKING_QUEUE)}


@app.post("/api/matchmaking/leave")
def matchmaking_leave(req: MatchmakingJoinRequest) -> dict[str, Any]:
    before = len(MATCHMAKING_QUEUE)
    MATCHMAKING_QUEUE[:] = [p for p in MATCHMAKING_QUEUE if p.get("telegram_id") != req.telegram_id]
    audit("matchmaking_leave", req.telegram_id, {"removed": before - len(MATCHMAKING_QUEUE)})
    return {"ok": True, "removed": before - len(MATCHMAKING_QUEUE), "queue_size": len(MATCHMAKING_QUEUE)}


@app.post("/api/matchmaking/fake_enemy")
def matchmaking_fake_enemy(req: MatchmakingJoinRequest) -> dict[str, Any]:
    MATCHMAKING_QUEUE[:] = [p for p in MATCHMAKING_QUEUE if p.get("telegram_id") != req.telegram_id]
    player_data = {
        "telegram_id": req.telegram_id,
        "username": req.username or "Player",
...
def finish_match(match_id: str) -> dict[str, Any]:
    match = ACTIVE_MATCHES.get(match_id)
    if not match:
        raise HTTPException(status_code=404, detail="Match not found")
    match["status"] = "finished"
    match["finished_at"] = datetime.now(timezone.utc).isoformat()
    audit("match_finished", "", {"match_id": match_id})
    return {"ok": True, "match": match}
