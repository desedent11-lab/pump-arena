...
function shell(content){ app.innerHTML = `<main class="app-shell${activeThemeClass()}"><section class="hero"><h1 class="logo">Pump Arena</h1><div class="subtitle">${lang ? t('tagline') : 'Crypto PvP battle prototype'}</div></section>${content}<div class="footer-note">MVP v7 · real battle · no install</div></main>`; }
...
function renderHome(){ if(!lang) return renderLanguage(); document.documentElement.dir=translations[lang].dir; shell(`<div class="card ${player.equipped.frame==='gold_profile'?'gold-frame':player.equipped.frame==='neon_frame'?'neon-frame':''}"><div class="stats"><div class="stat"><div class="label">💰 ${t('balance')}</div><div class="value">${player.balance} Credits</div></div><div class="stat"><div class="label">🏆 ${t('rank')}</div><div class="value">${player.rank}</div></div><div class="stat"><div class="label">⭐ ${t('level')}</div><div class="value">${player.level}</div></div><div class="stat"><div class="label">⚡ ${t('xp')}</div><div class="value">${player.xp}</div></div></div>${simpleClanStatusHtml()}<div class="equipped-line">${t('activeEffects')}: ${equippedText()}</div><div class="grid"><button class="menu-btn" onclick="startBattle()">${t('play')}</button><button class="menu-btn" onclick="claimDaily()">${t('daily')}</button><button class="menu-btn" onclick="renderRating()">${t('rating')}</button><button class="menu-btn" onclick="renderQuests()">${t('quests')}</button><button class="menu-btn" onclick="renderSeason()">${t('battlePass')}</button><button class="menu-btn" onclick="renderInventory()">${t('inventory')}</button><button class="menu-btn" onclick="renderShop()">${t('shop')}</button><button class="menu-btn" onclick="renderLanguage()">${t('language')}</button><button class="menu-btn" onclick="renderProfile()">${t('profile')}</button></div></div>`); }
