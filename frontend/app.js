...
function finishBattleManual(){
  if(!battle){
    renderHome();
    return;
  }
  const ok = confirm(lang === 'ru' ? 'Закончить текущий бой и выйти на главный экран?' : 'Finish current battle and return to home?');
  if(!ok) return;
  battle = null;
  renderHome();
}
...
function renderBattle(){
  ...
  shell(`<div class="card">
    <div class="row">
      ...
      <button class="menu-btn danger" onclick="finishBattleManual()">🏁 ${lang==='ru'?'Закончить бой':'Finish Battle'}</button><button class="menu-btn" onclick="renderHome()">${t('back')}</button>
    </div>
