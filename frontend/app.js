...
function finishBattleAndGoHomeV35(){
  try{
    if(typeof battle !== 'undefined' && battle){
      battle.ended = true;
      battle.reward = 0;
      battle.msg = lang === 'ru' ? 'Бой завершён игроком.' : 'Battle finished by player.';
      battle = null;
    }
    localStorage.removeItem('pump_active_battle');
    localStorage.removeItem('pump_current_battle');
...
function battleBackHomeV37(){
  finishBattleAndGoHomeV35();
}
...
function renderBattle(){
  ...
  shell(`<div class="card">
    <div class="row">
      ...
      <button class="menu-btn finish-battle-btn-v35" onclick="battleBackHomeV37()">🏁 ${lang==='ru'?'Закончить бой':'Finish Battle'}</button>
    </div>
    ...
    ${ended?`<h2>${t('result')}: ${battle.msg}</h2><p class="big">${t('reward')}: ${battle.reward>0?'+':''}${battle.reward} Credits</p><div class="grid"><button class="action-btn" onclick="startBattle()">${t('play')}</button><button class="action-btn finish-battle-btn-v35" onclick="battleBackHomeV37()">🏠 ${t('back')}</button></div>`:
    `<div class="grid">
      ...
      <button class="action-btn finish-battle-btn-v35" onclick="battleBackHomeV37()">🏁 ${lang==='ru'?'Закончить бой':'Finish Battle'}</button>
    </div>`}
    <h3>${L('battleLog')}</h3>
...
/* V36_FINISH_BATTLE_DELEGATE_START */
document.addEventListener('click', function(e){
  const btn = e.target && e.target.closest && e.target.closest('.finish-battle-btn-v35');
  if(!btn) return;
  e.preventDefault();
  e.stopPropagation();
  if(typeof finishBattleAndGoHomeV35 === 'function'){
    finishBattleAndGoHomeV35();
  }
}, true);
...
/* V36_FINISH_BATTLE_DELEGATE_END */


/* V37_BATTLE_EXIT_GUARD_START */
document.addEventListener('click', function(e){
  const btn = e.target && e.target.closest && e.target.closest('button');
  if(!btn) return;
  const txt = (btn.textContent || '').toLowerCase();
  const isBack = txt.includes('назад') || txt.includes('back') || txt.includes('закончить бой') || txt.includes('finish battle');
  if(!isBack) return;
  ...
  try{
    if(typeof battle !== 'undefined' && battle){
      battle.ended = true;
      battle.reward = 0;
      battle.msg = (typeof lang !== 'undefined' && lang === 'ru') ? 'Бой завершён.' : 'Battle finished.';
      battle = null;
      localStorage.removeItem('pump_active_battle');
      localStorage.removeItem('pump_current_battle');
      localStorage.removeItem('pump_v30_last_match');
