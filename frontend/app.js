...
function finishBattleAndGoHomeV35(){
  // Полностью завершаем текущий бой, чтобы он не продолжался после выхода.
  try{
    if(battle && !battle.ended){
      ...
      battle.log.unshift(lang === 'ru' ? '🏁 Бой завершён вручную' : '🏁 Battle finished manually');
    }
  }catch(e){}
...
function renderBattle(){
  ...
  shell(`<div class="card">
    <div class="row">
      ...
      <button class="menu-btn finish-battle-btn-v35" onclick="finishBattleAndGoHomeV35()">🏁 ${lang==='ru'?'Закончить бой':'Finish Battle'}</button>
    </div>
    ...
    ${ended?`<h2>${t('result')}: ${battle.msg}</h2><p class="big">${t('reward')}: ${battle.reward>0?'+':''}${battle.reward} Credits</p><div class="grid"><button class="action-btn" onclick="startBattle()">${t('play')}</button><button class="action-btn finish-battle-btn-v35" onclick="finishBattleAndGoHomeV35()">🏠 ${t('back')}</button></div>`:
    `<div class="grid">
      ...
      <button class="action-btn finish-battle-btn-v35" onclick="finishBattleAndGoHomeV35()">🏁 ${lang==='ru'?'Закончить бой':'Finish Battle'}</button>
    </div>`}
    <h3>${L('battleLog')}</h3>
