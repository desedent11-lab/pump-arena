...
/* V34_FINISH_BATTLE_FORCE_START */
window.finishBattleForceV34 = function(){
  try{
  ...
  const btn = document.getElementById("finishBattleFloatingBtnV34");
  if(btn) btn.remove();
...
(function(){
  function hasActiveBattle(){
    try{
      if(typeof battle !== "undefined" && battle && !battle.ended) return true;
    }catch(e){}

    const txt = (document.body && document.body.innerText || "").toLowerCase();
    return (
      txt.includes("pump battle") ||
      txt.includes("следующий тик") ||
      txt.includes("next tick") ||
      txt.includes("закрыть позицию") ||
      txt.includes("close position") ||
      txt.includes("whale push")
    );
  }

  function removeBtn(){
    const old = document.getElementById("finishBattleFloatingBtnV34");
    if(old) old.remove();
  }
  ...
  function addBtn(){
    ...
    let btn = document.getElementById("finishBattleFloatingBtnV34");
    if(!btn){
      btn = document.createElement("button");
      btn.id = "finishBattleFloatingBtnV34";
      btn.type = "button";
  ...
  function patchRenderBattle(){
    try{
      if(typeof renderBattle === "function" && !renderBattle.__v34Patched){
        const old = renderBattle;
        const wrapped = function(){
          const result = old.apply(this, arguments);
          setTimeout(addBtn, 0);
          setTimeout(addBtn, 120);
          setTimeout(addBtn, 350);
          return result;
        };
        wrapped.__v34Patched = true;
        renderBattle = wrapped;
      }
    }catch(e){}
  }
  ...
  function patchRenderHome(){
    try{
      if(typeof renderHome === "function" && !renderHome.__v34Patched){
  ...
  document.addEventListener("click", function(e){
    const target = e.target && e.target.closest && e.target.closest("#finishBattleFloatingBtnV34");
    if(target){
      e.preventDefault();
      e.stopPropagation();
      window.finishBattleForceV34();
